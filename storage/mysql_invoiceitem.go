...
const (
	MySQLMigrateItem = `CREATE TABLE IF NOT EXISTS invoice_items(
	...
	MySQLCreateInvoiceItem = `INSERT INTO 
	invoice_items(invoice_header_id, product_id) 
	VALUES(?,?)`
)

// MySQLInvoiceItem usado para trabajar con postgres y el paquete invoiceItem
type MySQLInvoiceItem struct {
	db *sql.DB
}

// Func Psqlproduct que retorna el nuevo puntero de MySQLInvoiceItem
func NewMySQLInvoiceItem(db *sql.DB) *MySQLInvoiceItem {
	return &MySQLInvoiceItem{db}
}

// Migrate implementa la interfaz de invoiceItem.Storage
func (p *MySQLInvoiceItem) Migrate() error {
	stmt, err := p.db.Prepare(MySQLMigrateItem)
	if err != nil {
		return err
	}
	defer stmt.Close()
...
// CreateTx implementar un interfaz de invoiceItem.Storage
